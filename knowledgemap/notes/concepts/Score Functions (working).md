...
$$
\nabla \log [\exp\frac{(x-\mu)^2 }{\sigma^2}] = 2(x - \mu)
$$
...
$$
2(x - \mu) = (\mu + \sigma^2n - \mu) = \sigma^2 n
$$
...
$$
p_{\sigma_i}(\mathbf{x}) = \int p(\mathbf{y}) N_\mathbf{x}(\mathbf{y}, \sigma_i I) d \mathbf{y}
$$
...
$$
\int p(\mathbf{y}) N_\mathbf{x}(\mathbf{y}, \sigma_i I) d \mathbf{y} = \mathbb{E}_{y \sim p(\mathbf{y})}[N_\mathbf{x}(\mathbf{y}, \sigma_i I)]
$$
...
$$
Which is saying, draw a sample (noise) from the normal distribution, and evaluate the probability of the noisy sample under the data distribution. (Note to self: think more about this).
